...
const CATALOG = {
  ...
  'usage.run.mark': { en: 'Mark runs', he: 'הרצות סימון' },
  'usage.run.enrichment': { en: 'Enrichment runs', he: 'הרצות העשרה' },
  'usage.run.adhoc': { en: 'Ad-hoc runs', he: 'הרצות אד-הוק' },
  ...
  'usage.shas.note': { en: 'Estimate: each producer’s avg $/priced call × how often it fires per amud × {amudim} amudim, grossed up ×{gross} for Workers AI (billed but unpriced per-producer). Coverage is uneven, so most of the remaining cost is the lightly-warmed long tail. The gateway total is authoritative for money actually spent.', he: 'אומדן: עלות ממוצעת לקריאה מתומחרת לכל מפיק × תדירות ההפעלה לעמוד × {amudim} עמודים, מוגדל פי {gross} עבור Workers AI (מחויב אך לא מתומחר ברמת המפיק). הכיסוי אינו אחיד, ולכן רוב העלות שנותרה היא הזנב הארוך שחומם מעט. סך ה-gateway הוא המקור הסמכותי להוצאה בפועל.' },
  ...
  'usage.latency.byMark': { en: 'Runs by mark', he: 'הרצות לפי סימון' },
  'usage.latency.byMark.hint': { en: 'rolled up across /api/run with mark_id', he: 'מסוכם על פני /api/run עם mark_id' },
  'usage.latency.byEnrichment': { en: 'Runs by enrichment', he: 'הרצות לפי העשרה' },
