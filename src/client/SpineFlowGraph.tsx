 ...
 * the next daf's box — not a separate text band. Routed through a right-side
 * lane (interval-coloured) so connectors never sit on top of each other.
 *
 * Reuses the daf reader's flow palette + title wrapping (ArgumentFlowGraph) so
 * the boxes look identical to the per-daf view. Hidden #spine page only.
 */
...
const NODE_W = 330, NODE_H = 44, ROW_GAP = 10;
const DAF_HEADER_H = 26, DAF_GAP = 14;
const TOP_PAD = 12, LEFT_PAD = 46;       // left room for the daf label
...
export default function SpineFlowGraph(props: { dapim: SpineViewDaf[] }): JSX.Element {
  const model = createMemo(() => {
    ...
    for (const d of props.dapim) {
      for (const c of d.flow) {
        ...
        if (c.from !== c.to && nodeY.has(from) && nodeY.has(to)) {
          edges.push({ from, to, kind: c.kind, cross: false, note: c.note, fromSec: c.from, toSec: c.to, fromPage: d.page, toPage: d.page });
        }
      }
      if (d.nextPage) {
        for (const e of d.cross) {
          ...
          if (nodeY.has(from) && nodeY.has(to)) {
            edges.push({ from, to, kind: e.relation as Kind, cross: true, note: e.note, fromSec: e.fromSection, toSec: e.toSection, fromPage: d.page, toPage: d.nextPage });
          }
        }
      }
    }

    const mid = (key: string) => (nodeY.get(key) ?? 0) + NODE_H / 2;
    ...
    return { nodeY, nodeTitle, nodeNum, dafHeaders, height, edges, lanes, width };
  });
  ...
  const edgePath = (y1: number, y2: number, lane: number): string => {
    ...
    return [
      `M ${rightX} ${y1}`, `L ${x - r} ${y1}`, `Q ${x} ${y1} ${x} ${y1 + dir * r}`,
      `L ${x} ${y2 - dir * r}`, `Q ${x} ${y2} ${x - r} ${y2}`, `L ${rightX} ${y2}`,
    ].join(' ');
  };
  ...
  return (
    <Show when={props.dapim.length > 0}>
      <div style={{ 'max-height': '78vh', 'overflow-y': 'auto', 'overflow-x': 'auto', border: '1px solid #ece9df', 'border-radius': '8px', background: '#fdfcf9', 'margin-top': '0.6rem', padding: '0.3rem' }}>
        {(() => {
          const m = model();
          const mid = (key: string) => (m.nodeY.get(key) ?? 0) + NODE_H / 2;
          return (
            <svg width={m.width} height={m.height} viewBox={`0 0 ${m.width} ${m.height}`} style={{ display: 'block' }}>
              ...
              <For each={m.edges}>{(e, i) => (
                <path
                  d={edgePath(mid(e.from), mid(e.to), m.lanes[i()])}
              ...
              <For each={[...m.nodeY.keys()]}>{(key) => {
                ...
                return (
                  <g>
                    ...
                    <rect x={LEFT_PAD} y={yTop} width={NODE_W} height={NODE_H} rx={10} ry={10} fill="#ffffff" stroke="#e4e0d4" stroke-width={1} filter="url(#spine-card-shadow)" />
                    <circle cx={LEFT_PAD + 18} cy={cy} r={11} fill="#f2eee4" stroke="#e4e0d4" stroke-width={1} />
                    <text x={LEFT_PAD + 18} y={cy} text-anchor="middle" dominant-baseline="central" font-size="11" font-weight="700" font-family="system-ui, sans-serif" fill="#8a2a2b">{num}</text>
                    <For each={lines}>{(line, li) => (
                      <text x={LEFT_PAD + 38} y={cy + (li() - (lines.length - 1) / 2) * LINE_H} text-anchor="start" dominant-baseline="central" font-size="12" font-weight="600" font-family="system-ui, sans-serif" fill="#2a2723">{line}</text>
                    )}</For>
                  </g>
                );
              }}</For>
