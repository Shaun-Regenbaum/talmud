...
interface SegInst { startSegIdx: number; endSegIdx: number; label: string }
interface NameInst { name: string; nameHe: string; generation: string; excerpt: string }
...
function adjPage(page: string, dir: 'prev' | 'next'): string | null {
  ...
  if (!m) return null;
  const n = Number(m[1]); const a = m[2].toLowerCase();
...
const GUTTER_KINDS = new Set(['argument', 'halacha', 'chart', 'aggadata', 'yerushalmi', 'rishonim', 'pesuk']);
const kindColor = (k: string): string => (GUTTER_KINDS.has(k) ? colorForKind(k as Parameters<typeof colorForKind>[0]) : k === 'rabbi' ? '#0066cc' : k === 'place' ? '#9a3412' : '#222');
...
export function AlignPage(): JSX.Element {
  ...
  const prevRef = createMemo(() => { const p = adjPage(page(), 'prev'); return p ? { t: tractate(), p } : null; });
  const nextRef = createMemo(() => { const p = adjPage(page(), 'next'); return p ? { t: tractate(), p } : null; });
  const [prevDaf] = createResource(prevRef, (r) => fetchJson<DafResp>(`/api/daf/${encodeURIComponent(r.t)}/${r.p}`).catch(() => ({})));
  const [nextDaf] = createResource(nextRef, (r) => fetchJson<DafResp>(`/api/daf/${encodeURIComponent(r.t)}/${r.p}`).catch(() => ({})));

  const items = () => ctx()?.items ?? [];
  const timing = () => ctx()?.timing ?? [];
  const allMarks = () => (marksRes()?.marks ?? []).filter((m) => m.cached && m.instances.length > 0);
  ...
  function instRowHtml(m: MarkRow, inst: SegInst, idx: number): string {
    const c = kindColor(m.kind); const range = rangeOfInst(inst);
  ...
  function detailHtml(): string {
    const d = detail();
    if (d?.t === 'src') {
      const it = byKey().get(d.key); if (!it) return '';
    ...
    if (d?.t === 'ent') {
      const e = entityIndex().get(d.key); if (!e) return '';
      return `<div class="aw-card aw-detail"><button class="aw-back" data-back>← back to list</button>
          ...
          <div class="aw-spinebox" ref={spineBox}>
            <Show when={adjPreview(prevDaf(), 'last', prevOpen())}>
              <div class={`aw-adj${hlAdj() === 'prev' ? ' hot' : ''}`} data-adj="prev" onClick={() => setPrevOpen((o) => !o)}>
                ...
                <div class="aw-adjtext" dir="rtl" innerHTML={adjPreview(prevDaf(), 'last', prevOpen())} />
              </div>
            </Show>
            ...
            <Show when={adjPreview(nextDaf(), 'first', nextOpen())}>
              <div class={`aw-adj${hlAdj() === 'next' ? ' hot' : ''}`} data-adj="next" onClick={() => setNextOpen((o) => !o)}>
                <div class="aw-adjlab">› next · {adjPage(page(), 'next')} · {nextOpen() ? 'full amud (collapse)' : 'first line (expand)'}</div>
                <div class="aw-adjtext" dir="rtl" innerHTML={adjPreview(nextDaf(), 'first', nextOpen())} />
              </div>
            </Show>
          </div>
