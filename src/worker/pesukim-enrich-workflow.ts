/**
 * Cloudflare Workflow: batch-enrich pesukim citations across a tractate.
 *
 * Each step processes ONE daf: hits POST /api/enrich-pesukim/:t/:p?strategy=...
 * for each requested strategy. The endpoint requires cached /api/pesukim
 * output; the workflow auto-discovers daf list from the pesukim:v1:* prefix.
 * Steps are durable. AI calls route through the AI Gateway.
 *
 * Trigger: POST /api/admin/enrich-pesukim-batch/{tractate}
 *          body: { dafim?, strategies?, refresh? }
 * Status:  GET  /api/admin/enrich-pesukim-batch/status/{instanceId}
 */
import { WorkflowEntrypoint, type WorkflowEvent, type WorkflowStep } from 'cloudflare:workers';

export type PesukimStrategy =
  | 'tanach-context' | 'peshat' | 'gemara-usage' | 'exegesis' | 'synthesize';

export interface PesukimEnrichParams {
  tractate: string;
  dafim?: string[];
  strategies?: PesukimStrategy[];
  refresh?: boolean;
  baseUrl?: string;
}
...
const ALL_STRATEGIES: readonly PesukimStrategy[] = [
  'tanach-context', 'peshat', 'gemara-usage', 'exegesis', 'synthesize',
];
const DEFAULT_BASE_URL = 'https://talmud.shaunregenbaum.com';

export class PesukimEnrichWorkflow extends WorkflowEntrypoint<Env, PesukimEnrichParams> {
  override async run(event: WorkflowEvent<PesukimEnrichParams>, step: WorkflowStep) {
    const {
      tractate,
      dafim: dafimParam,
    ...
    const dafim = await step.do('list-dafim', { retries: { limit: 3, delay: '10 seconds' } }, async () => {
      ...
      const prefix = `pesukim:v1:${tractate}:`;
      const out: string[] = [];
      ...
      await step.do(
        ...
        async () => {
          for (const strategy of strategies) {
            const url = `${baseUrl}/api/enrich-pesukim/${encodeURIComponent(tractate)}/${encodeURIComponent(daf)}?strategy=${strategy}${refresh ? '&refresh=1' : ''}`;
        ...
        console.warn(`[pesukim-enrich-workflow] ${tractate}/${daf} failed:`, String(err).slice(0, 300));
      });
    }
