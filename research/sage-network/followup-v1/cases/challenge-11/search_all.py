"""Exact-phrase Sefaria search across all corpora; saves raw response and logs it."""
import datetime, hashlib, json, os, sys, urllib.request
phrase, name = sys.argv[1], sys.argv[2]
body = {"query": phrase, "type": "text", "field": "exact", "size": 100, "slop": 0, "source_proj": True}
...
try:
    ...
    d = json.loads(data)
    print("total", d["hits"].get("total"))
    for h in d["hits"]["hits"]:
        print(h["_source"].get("ref"), "|", h["_source"].get("version"), "|", h["_source"].get("exact", "")[:0])
