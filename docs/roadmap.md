...
- **Finish splitting `index.ts`.** The Talmud worker's entry file was one file with about 100 routes. Most route families now live in `packages/talmud/src/worker/routes/`, and `tests/worker-route-table.test.ts` pins the registration order so a move cannot reorder or drop one. What is left needs the run engine untangled first: the spine and flow reads, the daf-index/daf-view/daf-runs group, and the run-tree endpoint all sit on `readCachedResult`, `readMarkInstances` and the cron helpers, which still live in the entry file.
