...
const CHULLIN_20A = daf([
  ['ואי', 0], ['סד', 0], ['מחזיר', 0], ['דוקא', 0],
  ['מאי', 1], ['איריא', 1], ['מולק', 1],
  ['אר', 2], ['ינאי', 2], ['יקבלו', 2], ['הרובין', 2],
]);

describe('injectAnchorMarkers seg fallback', () => {
  it('uses a verbatim excerpt match when the phrase is present', () => {
    const out = injectAnchorMarkers(CHULLIN_20A, [{ excerpt: 'מחזיר דוקא', index: 0, startSegIdx: 0 }], 'daf-argument-anchor');
    const doc = parse(out);
  ...
  it('falls back to the startSegIdx position when the excerpt does not match', () => {
    ...
    const out = injectAnchorMarkers(CHULLIN_20A, [{ excerpt: 'ואי סלקא דעתך מחזיר דוקא', index: 0, startSegIdx: 0 }], 'daf-argument-anchor');
    const doc = parse(out);
    const markers = doc.querySelectorAll('.daf-argument-anchor');
    expect(markers.length).toBe(1);
    const m = markers[0];
  ...
  it('anchors via segment even when the instance has no excerpt at all', () => {
    const out = injectAnchorMarkers(CHULLIN_20A, [{ excerpt: '', index: 2, startSegIdx: 2 }], 'daf-argument-anchor');
    const doc = parse(out);
  ...
  it('renders no marker when neither the excerpt nor a known segment resolves', () => {
    const out = injectAnchorMarkers(CHULLIN_20A, [{ excerpt: 'פסוק שאינו קיים', index: 0, startSegIdx: 99 }], 'daf-argument-anchor');
...
describe('range anchors (aggadata / pesukim) seg fallback', () => {
  const STORY = daf([
    ['פתיחה', 0], ['של', 0], ['הסיפור', 0],
    ['אמצע', 1], ['הסיפור', 1],
    ['סוף', 2], ['הסיפור', 2], ['כאן', 2],
  ]);
...
describe('injectOpinionMarkers section bounding via segment', () => {
  ...
  const HTML = daf([
    ['פתח', 0], ['רבי', 0], ['מאיר', 0], ['אומר', 0], ['כך', 0],
    ['רבי', 1], ['יהודה', 1], ['אומר', 1], ['אחרת', 1],
  ]);
