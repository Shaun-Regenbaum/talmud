...
let runCacheKey: (e: string, t: string, p: string, i: string) => string;
...
describe('runCacheKey', () => {
  it('joins enrichmentId:tractate:page:instanceKey', () => {
    expect(runCacheKey('argument.synthesis', 'Berakhot', '2a', '0-4-Opening')).toBe(
      'argument.synthesis:Berakhot:2a:0-4-Opening',
    );
  });

  it('is distinct when any component differs', () => {
    const keys = new Set([
      runCacheKey('rabbi.synthesis', 'Berakhot', '2a', 'Rabbi Eliezer'),
      runCacheKey('rabbi.relationships', 'Berakhot', '2a', 'Rabbi Eliezer'), // enrichment
      runCacheKey('rabbi.synthesis', 'Shabbat', '2a', 'Rabbi Eliezer'),       // tractate
      runCacheKey('rabbi.synthesis', 'Berakhot', '2b', 'Rabbi Eliezer'),      // page
      runCacheKey('rabbi.synthesis', 'Berakhot', '2a', 'Rabbi Akiva'),        // instance
    ]);
...
describe('runResultCache — store / retrieve', () => {
  it('round-trips a result under its key (a re-click hits the memo)', () => {
    const key = runCacheKey('pesukim.synthesis', 'Berakhot', '2a', 'Deuteronomy 6:7');
  ...
  it('does not collide across instances of the same enrichment', () => {
    const k1 = runCacheKey('pesukim.synthesis', 'Berakhot', '2a', 'Deuteronomy 6:7');
    const k2 = runCacheKey('pesukim.synthesis', 'Berakhot', '2a', 'Leviticus 22:7');
    ...
    expect(runResultCache.has(runCacheKey('pesukim.synthesis', 'Berakhot', '2a', 'Genesis 1:5'))).toBe(false);
  });
});

describe('cache invalidation', () => {
  it('clearRunResultCache() empties the memo', () => {
    runResultCache.set(runCacheKey('a', 'B', '2a', 'x'), fakeResult('one'));
    runResultCache.set(runCacheKey('b', 'B', '2a', 'y'), fakeResult('two'));
  ...
  it('a `marks-runs-invalidate` event clears the memo (model/prompt change)', () => {
    runResultCache.set(runCacheKey('rabbi.synthesis', 'B', '2a', 'Hillel'), fakeResult('bio'));
