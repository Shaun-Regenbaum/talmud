...
describe('Hebrew prompt coverage — prose enrichments never silently fall back to English', () => {
  for (const id of MUST_BE_BILINGUAL) {
    it(`${id} has a Hebrew system prompt + user template`, () => {
      ...
      expect((ext.system_prompt_he ?? '').trim().length, `${id} missing system_prompt_he`).toBeGreaterThan(0);
      expect((ext.user_prompt_template_he ?? '').trim().length, `${id} missing user_prompt_template_he`).toBeGreaterThan(0);
    });
  }
  ...
  it('every Hebrew prompt is actually Hebrew (contains Hebrew letters, not a copied English string)', () => {
    const hebrew = /[֐-׿]/;
    for (const id of MUST_BE_BILINGUAL) {
      expect(hebrew.test(hePrompts(id)?.system_prompt_he ?? ''), `${id} system_prompt_he has no Hebrew letters`).toBe(true);
    }
  });
});
