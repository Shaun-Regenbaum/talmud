...
import {
  ...
  bodyText, text, txt, extractRefs, type HTMLElement,
} from './common.ts';
...
function parseSpeaker(label: string): DafyomiPointsEntry['speaker'] {
  const inner = label.replace(/^\(/, '').replace(/\)?:?\s*$/, '').trim();
...
export function parseIndentedEntries(content: HTMLElement, opts: EntryParseOpts = {}): SplitEntries {
  ...
  const pushTop = (e: DafyomiEntry) => { (amud === 'a' ? topA : topB).push(e); };
