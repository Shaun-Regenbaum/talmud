 ...
 */
export function findOptimalAlignment(
  sefariaWords: string[],
  hebrewBooksWords: string[],
  ...
  while (
    sefariaIdx < Math.min(maxLength, sefariaWords.length) &&
...
export function getSegmentsFromSelection(
  ...
  for (const segment of segmentMappings) {
    ...
    if (
      (selection.startWordIndex >= segment.startWordIndex &&
        selection.startWordIndex <= segment.endWordIndex) ||
      (selection.endWordIndex >= segment.startWordIndex &&
        selection.endWordIndex <= segment.endWordIndex) ||
      (selection.startWordIndex <= segment.startWordIndex &&
        selection.endWordIndex >= segment.endWordIndex)
    ) {
...
export function findCorrespondingText(
  ...
  const targetWords =
    targetType === 'sefaria' ? alignment.sefariaWords : alignment.hebrewBooksWords;
