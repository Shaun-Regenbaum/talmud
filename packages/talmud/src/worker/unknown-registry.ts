...
const RABBI_TITLE_TOKENS = new Set([
  'רב',
  'רבי',
  'רבא',
  'רבה',
...
function isEthnonym(name?: string, nameHe?: string): boolean {
  ...
  return /^(aramean|aramaean|samaritan|cuthean|gentile|heathen|idolater)/i.test(
    (name || '').trim(),
  );
}
...
async function bump<
  T extends { firstSeen: number; lastSeen: number; count: number; dafs: string[] },
...
async function bumpBatch<
  T extends { firstSeen: number; lastSeen: number; count: number; dafs: string[] },
>(cache: KVNamespace, entries: Array<{ key: string; seed: () => T; daf: string }>): Promise<void> {
  const byKey = new Map<string, { seed: () => T; dafs: string[] }>();
  for (const e of entries) {
    if (!e.daf) continue;
    const g = byKey.get(e.key);
    if (g) {
      if (!g.dafs.includes(e.daf)) g.dafs.push(e.daf);
  ...
  for (const [key, g] of byKey) {
    try {
      ...
      for (const d of g.dafs)
        if (!rec.dafs.includes(d) && rec.dafs.length < MAX_DAFS) rec.dafs.push(d);
      await cache.put(key, JSON.stringify(rec), { expirationTtl: TTL_S });
    } catch (err) {
...
export function putObservedConceptsBatch(
  ...
  const now = Date.now();
  return bumpBatch<ObservedConcept>(
    cache,
    items.flatMap((c) => {
      const keyPart = norm(c.termHe || '') || norm(c.term || '');
      if (!keyPart) return [];
...
export function putObservedPlacesBatch(
  ...
  const now = Date.now();
  return bumpBatch<ObservedPlace>(
    cache,
    items.flatMap((p) => {
...
async function listPrefix<T extends { count: number }>(
  ...
  const ingest = async (names: string[]): Promise<void> => {
    for (let i = 0; i < names.length; i += LIST_BATCH) {
      ...
      if (top.length > keepCap) {
        top.sort(byCount);
        top = top.slice(0, sample);
      }
    }
  };
...
export function listObservedPlaces(
  cache: KVNamespace,
  sample = 50,
): Promise<UnknownSummary<ObservedPlace>> {
  ...
  return listPrefix<ObservedPlace>(cache, PLACE_PREFIX, sample, (r) =>
    isRealPlace(r.name, r.nameHe),
  );
}

export function listObservedConcepts(
  cache: KVNamespace,
  sample = 50,
): Promise<UnknownSummary<ObservedConcept>> {
