...
import { ARGUMENT_BRIDGE_OUTPUT_SCHEMA, ARGUMENT_CROSS_FLOW_OUTPUT_SCHEMA } from './output-schemas';
...
import { registerDafGenerateRoutes, startDafGeneration } from './routes/daf-generate';
...
import { recordTelemetry, runTelemetryRec } from './telemetry';
import type { Bindings, JobMessage, WaitUntilCtx } from './types';
import { recordObservedConcept, recordObservedPlace, recordUnknownRabbi } from './unknown-registry';
import { recordUsage } from './usage-rollup';
import { buildLearnedAdjacency, type VoiceGraphBlob } from './voice-graph';
import { runWarmCron } from './warm-cron';
import { checkWholeDafLeakAndAlert } from './whole-daf-leak-watch';
import { checkWorkerHealthAndAlert } from './worker-health';
import {
  createDagPool,
  type DafWarmParams,
  dafGenSentinelKey,
...
// The Workflow trigger and POST /api/daf-generate now live in
// ./routes/daf-generate.
registerDafGenerateRoutes(app);
...
// The queue-failure ring buffer read and the per-rabbi observation index now
// live in ./routes/observations.
registerObservationRoutes(app);

// --- Admin: spend + reachability ------------------------------------------
// The LLM cost ledger, the budget read/reset and the HebrewBooks probe now
// live in ./routes/admin-cost.
...
// --- Admin: cache + warm ops, and the client log sink ---------------------
// Warm status, strip-ttl, cache stats, cache GC and the /api/log pair now
// live in ./routes/admin-ops.
registerAdminOpsRoutes(app);

// --- Q&A on a move ------------------------------------------------------
// The /api/qa/* endpoints and their KV registry now live in ./routes/qa.
...
// --- Usage dashboard + bug reports --------------------------------------
// POST /api/report, the dashboard section builders and every /api/usage*
// endpoint (plus /api/shas-cost, /api/worker-health and
// /api/admin/report-dismiss) now live in ./routes/usage.
...
// --- Rabbi/sage admin -----------------------------------------------------
// The per-sage enrichment endpoints, the global compile endpoints and the
// reads over their blobs now live in ./routes/rabbi-admin. The shared
// rabbi-identification helpers above stay here.
registerRabbiAdminRoutes(app);

// --- Daf-scoped: Region + Mesorah ---------------------------------------
// Both first-pass reads over the cached skeleton now live in
// ./routes/region-mesorah.
registerRegionMesorahRoutes(app);

// --- Admin: Hebrew-Wikipedia bio → English summary ----------------------
// POST /api/admin/translate-bio now lives in ./routes/admin-translate-bio.
registerAdminTranslateBioRoutes(app);
...
export const DEEP_WARM_PLAN: Record<string, string[]> = {
...
// Whole-daf enrichments warmed by deepWarmDaf, in dependency-friendly order
// (deps listed before the chips that consume them). Experimental, LLM-backed
// members (e.g. biyun.essay) are kept in this list so an EXPLICIT re-warm
// cascade can still target them, but the default neighbour deep-warm skips them
// via isExperimentalLlmWarm — only visible (non-experimental) producers are
// generated by default, the same gate /api/run enforces for the lazy path.
export const WHOLE_DAF_WARM_ENRICHMENTS = [
...
async function deepWarmDaf(
  ...
  for (const eid of WHOLE_DAF_WARM_ENRICHMENTS) {
    if (!wanted(eid)) continue;
    // Experimental, LLM-backed enrichments (biyun.essay) are dev-only and must
    // never be generated by a default neighbour deep-warm — same gate /api/run
    // enforces for the lazy path. An explicit re-warm cascade (`only` set, e.g.
    // POST /api/admin/rewarm) is a deliberate trusted action and still warms it.
    if (!only && isExperimentalLlmWarm({ enrichment_id: eid })) continue;
    try {
