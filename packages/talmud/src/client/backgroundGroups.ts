...
export function orderBackgroundGroups(groups: BackgroundGroup[] | undefined): BackgroundGroup[] {
  ...
  for (const g of groups) {
    ...
    const terms = g.terms.filter(
      (tm) => tm && typeof tm.term === 'string' && tm.term.trim()
        && typeof tm.gloss === 'string' && tm.gloss.trim(),
    );
  ...
  return BACKGROUND_CATEGORY_ORDER.filter((c) => byCat.has(c)).map((c) => ({ category: c, terms: byCat.get(c)! }));
}
