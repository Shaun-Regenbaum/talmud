...
import { colorForKind, GutterGlyph, type GutterKind } from './GutterIcons';
import { colorForGeneration, type GenerationId, legibleTextColor } from './generations';
...
export function TutorialCoach(): JSX.Element {
  ...
  const readVp = () => {
    ...
    setVp(
      v
        ? { top: v.offsetTop, height: v.height, scale: v.scale }
        : { top: 0, height: window.innerHeight, scale: 1 },
    );
  };
  ...
  const measure = (attempt = 0, fromObserver = false) => {
    ...
    else if (typeof s.selectorIndex === 'number')
      targets = [all[Math.max(0, Math.min(s.selectorIndex, all.length - 1))]];
  ...
  const reposition = () => {
    if (isMobile()) {
      setPos(null);
      return;
    } // mobile uses fixed sheets, not side math
    const r = rect();
    const vw = window.innerWidth;
    const vh = window.innerHeight;
    if (!r || !cardEl) {
      setPos(r ? null : { mode: 'center' });
      return;
    }
    ...
    const clampTop = (y: number) =>
      Math.max(PAD, Math.min(y, vh - Math.min(ch, vh - 2 * PAD) - PAD));
    ...
    if (vspace >= MIN_VERT) {
      ...
      setPos({
        ...
        side: vside,
        top: clampTop(top),
        left: clampLeft(r.left + r.width / 2 - cw / 2),
        maxH: vspace,
      });
      return;
    }
    ...
    if (hspace >= MIN_HORIZ) {
      ...
      setPos({
        mode: 'anchor',
        side: hside,
        top: clampTop(r.top + r.height / 2 - ch / 2),
        left: leftX,
        maxH: vh - 2 * PAD,
      });
      return;
  ...
  createEffect(() => {
    ...
    if (s.note !== prevNote) {
      if (s.note) openTutorialNote(s.note);
    ...
    if (s.expandQa) {
      const tryExpand = (n = 0) => {
        const toggle = document.querySelector<HTMLButtonElement>(
          '[data-tour="argument-qa"] button[aria-expanded]',
        );
        if (toggle) {
  ...
  onCleanup(() => {
    ...
    setTutorialHeader(false);
    triggerTutorialTranslate('clear');
    setTutorialPrefetch(false);
    setTutorialNoteInteractive(false);
  });
  ...
  const cardStyle = (): JSX.CSSProperties => {
    ...
    if (isMobile()) {
      ...
      if (shifted) {
        ...
        return {
          ...
          ...(atTop
            ...
            : {
                top: `${Math.round(v.top + v.height - MOBILE_BAR)}px`,
      ...
      return {
        ...
        left: '8px',
        right: '8px',
        [atTop ? 'top' : 'bottom']: atTop
          ? 'calc(8px + env(safe-area-inset-top))'
          : `calc(${MOBILE_BAR}px + env(safe-area-inset-bottom))`,
        'max-height': '46vh',
        'border-radius': '12px',
        overflow: 'hidden',
      };
    ...
    return {
      ...
      ...(anchored
        ? {
            ...
            'max-height': `${Math.max(MIN_VERT, anchored.maxH)}px`,
          }
        : {
            top: '50%',
            left: '50%',
  ...
  return (
    <Portal>
      ...
      <div
        aria-hidden="true"
        style={{ position: 'fixed', inset: '0', 'z-index': '5999', background: 'transparent' }}
      />
      ...
      <div ref={cardEl} style={cardStyle()}>
        ...
        <div
          style={{ padding: '14px 20px 22px', 'border-top': '1px solid #f1efea', flex: '0 0 auto' }}
        >
          ...
          >
            <Show when={!isLast()} fallback={<span />}>
              <button type="button" onClick={finish} style={linkBtn()}>
                {t('tutorial.skip')}
              </button>
            </Show>
...
const MARK_KINDS: { kind: GutterKind; labelKey: string; descKey: string }[] = [
  {
    kind: 'argument',
    labelKey: 'tutorial.icon.argument.label',
    descKey: 'tutorial.icon.argument.desc',
  },
  {
    kind: 'halacha',
    labelKey: 'tutorial.icon.halacha.label',
    descKey: 'tutorial.icon.halacha.desc',
  },
  {
    kind: 'aggadata',
    labelKey: 'tutorial.icon.aggadata.label',
    descKey: 'tutorial.icon.aggadata.desc',
  },
  {
    kind: 'yerushalmi',
    labelKey: 'tutorial.icon.yerushalmi.label',
    descKey: 'tutorial.icon.yerushalmi.desc',
  },
  { kind: 'pesuk', labelKey: 'tutorial.icon.pesuk.label', descKey: 'tutorial.icon.pesuk.desc' },
  {
    kind: 'rishonim',
    labelKey: 'tutorial.icon.rishonim.label',
    descKey: 'tutorial.icon.rishonim.desc',
  },
];
              ...
              <div style={{ display: 'flex', 'align-items': 'flex-start', gap: '10px' }}>
                <span
                  ...
                  style={{
                    ...
                    flex: '0 0 auto',
                    'margin-top': '1px',
                    background: colorForKind(m.kind),
                    color: '#fff',
                  }}
                >
...
function SpectrumRow(props: { labelKey: string; ids: GenerationId[] }): JSX.Element {
  return (
    <div>
      <div style={{ 'font-size': '12px', color: 'var(--muted, #6b7280)', 'margin-bottom': '5px' }}>
        {t(props.labelKey)}
      </div>
