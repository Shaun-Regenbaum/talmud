...
export default function ChecksPanel(props: { tractate: string; page: string }): JSX.Element {
  const [data] = createResource(
    ...
    async (): Promise<ChecksResponse | null> => {
      const r = await fetch(
        `/api/checks/${encodeURIComponent(props.tractate)}/${encodeURIComponent(props.page)}`,
      );
      if (!r.ok) return null;
          ...
          {(m) => (
            <div style={{ 'margin-bottom': '0.35rem' }}>
              <div style={{ 'font-weight': 600, color: '#444', 'margin-bottom': '0.1rem' }}>
                {m.mark_id}{' '}
                <span style={{ color: '#a16207', 'font-weight': 400 }}>· {m.issues.length}</span>
              </div>
              <For each={m.issues.slice(0, 12)}>
                {(i) => (
                  <div
                    style={{
                      display: 'flex',
                  ...
                  >
                    ...
                    <Show when={typeof i.index === 'number' && i.index >= 0}>
                      <span style={{ 'flex-shrink': 0, color: '#999', 'font-size': '0.68rem' }}>
                        seg {i.index}
                      </span>
                    </Show>
                    <Show when={i.match}>
              ...
              <Show when={m.issues.length > 12}>
                <div style={{ color: '#999', 'font-size': '0.68rem' }}>
                  +{m.issues.length - 12} more
                </div>
              </Show>
            </div>
          )}
