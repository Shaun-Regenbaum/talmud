...
async function probe(
  model: LLMModelId,
): Promise<{ ms: number; transport: string; reply: string; error?: string }> {
...
export default function SettingsPage() {
  ...
  const [probeResult, setProbeResult] = createSignal<
    Record<string, { ms: number; transport: string; reply: string; error?: string }>
  >({});
  ...
  return (
    <div
      class="page-shell"
      style={{ '--page-max': '880px', 'font-family': 'system-ui, sans-serif' }}
    >
      <h1 style={{ 'font-size': '24px', 'margin-bottom': '8px' }}>{t('settings.title')}</h1>
      <p style={{ color: '#666', margin: '0 0 24px' }}>
        {t('settings.intro.before')}
        <code>?model=</code>
        {t('settings.intro.after')}
      </p>
            ...
            <>
              <section style={{ 'margin-bottom': '32px' }}>
                <h2 style={{ 'font-size': '16px', 'margin-bottom': '8px' }}>
                  {t('settings.section.defaultModel')}
                </h2>
                <div style={{ 'font-family': 'monospace', 'font-size': '14px' }}>
                  {label(s.defaultModel)}
                  <span style={{ 'margin-left': '8px', color: '#999', 'font-size': '12px' }}>
                    {t('settings.source', { source: s.source })}
                  </span>
              ...
              <section style={{ 'margin-bottom': '32px' }}>
                <h2 style={{ 'font-size': '16px', 'margin-bottom': '8px' }}>
                  {t('settings.section.fallbackChain')}
                </h2>
                <p style={{ color: '#666', 'font-size': '13px', margin: '0 0 12px' }}>
                  {t('settings.fallbackChain.hint')}
                </p>
                <Show
                  when={s.fallbackChain.length > 0}
                  fallback={
                    <div style={{ color: '#999', 'font-style': 'italic' }}>
                      {t('settings.fallbackChain.empty')}
                    </div>
                  }
                >
                  <ol style={{ 'padding-left': '20px', margin: 0 }}>
                    <For each={s.fallbackChain}>
                      {(id) => (
                ...
                <For each={presets}>
                  {(p) => (
                    <div style={{ 'margin-bottom': '10px' }}>
                      ...
                      <span style={{ color: '#999', 'font-size': '13px', 'margin-left': '6px' }}>
                        {p.vendor}
                      </span>
                      {probeBtn(p.id)}
                      <Show when={p.notes}>
      ...
      <Show when={data.error}>
        <div style={{ color: '#c00' }}>
          {t('settings.loadFailed', { error: String(data.error) })}
        </div>
      </Show>
    </div>
  );
