...
describe('previousVersionKey — stale-while-revalidate key', () => {
  it('decrements a numeric enrichment version in the canonical key', () => {
    expect(
      previousVersionKey(
        'enrich:argument.background:5:abc:berakhot:2a',
        'argument.background',
        '5',
      ),
    ...
    expect(
      previousVersionKey(
        'enrich:argument-overview.synthesis:3:abc:berakhot:2a',
  ...
  it('handles :he and global keys (marker is in the prefix)', () => {
    expect(
      previousVersionKey(
        'enrich:argument.background:5:he:abc:berakhot:2a',
        'argument.background',
        '5',
      ),
    ).toBe('enrich:argument.background:4:he:abc:berakhot:2a');
    expect(previousVersionKey('enrich:rabbi.bio:3:abae', 'rabbi.bio', '3')).toBe(
      'enrich:rabbi.bio:2:abae',
    );
  });
});
