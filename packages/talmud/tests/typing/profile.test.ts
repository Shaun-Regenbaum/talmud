...
import {
  ...
  hasOpposingVoices,
  type LayerInstance,
  overlayUncoveredSegs,
  PRIMARY_FLOOR,
  registerOf,
  type UnitRange,
} from '../../src/lib/typing/profile';

const unit = (startSegIdx: number, endSegIdx: number): UnitRange => ({
  tractate: 'Gittin',
  page: '67b',
  startSegIdx,
  endSegIdx,
...
const inst = (
  ...
  e: number,
  confidence?: number,
): LayerInstance => ({ layer, instanceId, startSegIdx: s, endSegIdx: e, confidence });
...
describe('register (mishnah/gemara axis)', () => {
  ...
  it('is orthogonal to primary: a mishnah unit can be halacha-primary', () => {
    const p = composeTypeProfile(unit(0, 4), [inst('halacha', 'h', 0, 4)], {
      mishnaSegs: new Set([0, 1, 2, 3, 4]),
    });
    expect(p.primary).toBe('halacha');
...
describe('isDispute', () => {
  ...
  it('is false with no opposition or no voices supplied', () => {
    expect(hasOpposingVoices({ edges: [{ kind: 'supports' }, { kind: 'responds-to' }] })).toBe(
      false,
    );
  ...
  it('requires a named speaker: an opposes edge on an anonymous section is NOT a dispute', () => {
    ...
    const p = composeTypeProfile(unit(0, 0), [], {
      voices: { edges: [{ kind: 'opposes' }] },
      hasNamedSpeaker: false,
    });
  ...
  it('a named speaker + opposition IS a dispute', () => {
    const p = composeTypeProfile(unit(0, 4), [], {
      voices: { edges: [{ kind: 'opposes' }] },
...
describe('overlayUncoveredSegs (P0 coverage)', () => {
  it('returns unit segments covered by no content overlay (pure dialectic)', () => {
    ...
    const segs = overlayUncoveredSegs(unit(0, 5), [
      inst('aggadata', 'a', 0, 2),
      inst('rabbi', 'r', 4, 4),
    ]);
...
describe('named validation cases from the design doc', () => {
  ...
  it('כולכם unit (dispute, no content overlay) → pure-dialectic primary but isDispute', () => {
    const p = composeTypeProfile(unit(0, 2), [inst('argument', 'sec', 0, 2)], {
      voices: { edges: [{ kind: 'opposes' }, { kind: 'supports' }] },
    });
    expect(p.primary).toBe('pure-dialectic');
    expect(p.isDispute).toBe(true);
  });
  ...
  it('Berakhot 2a Stam Q&A segments → pure-dialectic, not untyped', () => {
    ...
    const p = composeTypeProfile(
      { tractate: 'Berakhot', page: '2a', startSegIdx: 5, endSegIdx: 6 },
      [],
    );
    expect(p.primary).toBe('pure-dialectic');
    expect(p.isDispute).toBe(false);
