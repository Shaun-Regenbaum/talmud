...
const PASS2_INVERTED_SWAP: Array<[string, string]> = [
  ...
  // Multi-word translit.
  [
    'gezera shava (verbal analogy from a shared word) ties them',
    'verbal analogy from a shared word (גזרה שוה) ties them',
  ],
...
const HEBRAIZE_FULL: Array<[string, string]> = [
  ...
  ['', ''],
  [
    'plain English with no Hebrew or parens at all',
    'plain English with no Hebrew or parens at all',
  ],
];
...
describe('hebraize — full pipeline catches bare-word authorities', () => {
  it('hebraicizes Rambam, Mishneh Torah, Tur, Shulchan Aruch, Rosh in halacha prose', () => {
    const input =
      'Rambam in Mishneh Torah codifies the position; the Tur and Shulchan Aruch follow; Rosh dissents.';
