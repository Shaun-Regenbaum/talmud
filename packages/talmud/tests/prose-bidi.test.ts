...
describe('bidiSegments — keep a quoted Hebrew/Aramaic citation in one bidi run', () => {
  ...
  it('keeps a closing ASCII quote OUT of the run (it keeps its English position)', () => {
    ...
    expect(heRuns("It insists: 'שמע מינה שעורא דעני לחוד וקודם.' The dispute")).toEqual([
      'שמע מינה שעורא דעני לחוד וקודם',
    ]);
  });
...
describe('stripEchoParens — drop redundant all-Hebrew gloss parentheticals', () => {
  it('drops an all-Hebrew paren right after a Hebrew term (the broken case)', () => {
    ...
    expect(
      stripEchoParens('a knife of מלא צואר וחוץ לצואר (מלא צואר וחוץ לצואר וחוץ לצואר) suffices'),
  ...
  it('KEEPS a real English gloss', () => {
    expect(stripEchoParens('valid שחיטה (ritual slaughter) requires')).toBe(
      'valid שחיטה (ritual slaughter) requires',
    );
  });
  ...
  it('KEEPS a quoted paren that adds a new word (not a pure repetition)', () => {
    ...
    expect(stripEchoParens("a knife 'מלא צואר' (מלא צואר מבחוץ) here")).toBe(
      "a knife 'מלא צואר' (מלא צואר מבחוץ) here",
    );
  });
  it('drops a quote-wrapped echo, keeping the closing quote (the production case)', () => {
    ...
    expect(
      stripEchoParens(
        "says the knife must be 'מלא צואר וחוץ לצואר' (מלא צואר וחוץ לצואר). The Gemara",
      ),
