...
import { recordObservedConcept, listObservedConcepts } from '../src/worker/unknown-registry';
...
function collector() {
  ...
  return { ctx, settle: async () => { await Promise.all(pending); pending = []; } };
}
...
describe('observed-concept backlog', () => {
  it('accumulates sightings of the same term (keyed on Hebrew) across dapim', async () => {
    ...
    recordObservedConcept({ CACHE }, ctx, { term: 'Kohen', termHe: 'כהן', gloss: 'A priest.', category: 'realia', tractate: 'Berakhot', page: '2a' });
    await settle();
    recordObservedConcept({ CACHE }, ctx, { term: 'priest', termHe: 'כהן', gloss: 'A descendant of Aharon.', category: 'realia', tractate: 'Shabbat', page: '5b' });
    await settle();
    recordObservedConcept({ CACHE }, ctx, { term: 'kohanim', termHe: 'כהן', gloss: 'The priestly class.', category: 'realia', tractate: 'Shabbat', page: '5b' });
    await settle();
  ...
  it('keeps distinct terms separate and ranks by sighting count', async () => {
    ...
    recordObservedConcept({ CACHE }, ctx, { term: 'Eruv', termHe: 'עירוב', gloss: 'g', tractate: 'Eruvin', page: '2a' });
    recordObservedConcept({ CACHE }, ctx, { term: 'Eruv', termHe: 'עירוב', gloss: 'g', tractate: 'Eruvin', page: '3a' });
    recordObservedConcept({ CACHE }, ctx, { term: 'Terumah', termHe: 'תרומה', gloss: 'g', tractate: 'Berakhot', page: '2a' });
    await settle();
  ...
  it('ignores terms with no term/termHe and is a no-op without a CACHE binding', async () => {
    const CACHE = fakeKV();
    const { ctx, settle } = collector();
    recordObservedConcept({ CACHE }, ctx, { term: '', termHe: '', gloss: 'x', tractate: 'Berakhot', page: '2a' });
    recordObservedConcept({}, ctx, { term: 'Kohen', termHe: 'כהן', gloss: 'x', tractate: 'Berakhot', page: '2a' });
    await settle();
    const out = await listObservedConcepts(CACHE);
