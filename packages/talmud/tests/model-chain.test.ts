...
describe('resolveChain — code-driven, no KV', () => {
  it('pins an explicit model and appends no default fallback', () => {
    expect(resolveChain({} as LLMEnv, opts({ model: 'openrouter/x/y' })))
      .toEqual(['openrouter/x/y']);
  });

  it('appends the caller-supplied fallback to a pinned model', () => {
    expect(resolveChain({} as LLMEnv, opts({ model: 'openrouter/x/y', fallback: ['@cf/a/b'] })))
      .toEqual(['openrouter/x/y', '@cf/a/b']);
  });

  it('falls to DEFAULT_MODEL + DEFAULT_FALLBACK_CHAIN when nothing is set', () => {
    expect(resolveChain({} as LLMEnv, opts({})))
      .toEqual([DEFAULT_MODEL, ...DEFAULT_FALLBACK_CHAIN]);
  });

  it('honors a valid DEFAULT_LLM_MODEL env override', () => {
    expect(resolveChain({ DEFAULT_LLM_MODEL: 'openrouter/foo/bar' } as LLMEnv, opts({})))
      .toEqual(['openrouter/foo/bar', ...DEFAULT_FALLBACK_CHAIN]);
  });

  it('ignores an invalid env value and uses DEFAULT_MODEL', () => {
    expect(resolveChain({ DEFAULT_LLM_MODEL: 'not-a-model' } as LLMEnv, opts({})))
      .toEqual([DEFAULT_MODEL, ...DEFAULT_FALLBACK_CHAIN]);
  });
