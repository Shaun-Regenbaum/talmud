...
import { lintCalques, lintSynthesis } from '../src/lib/synthesisLint';
...
describe('lintSynthesis — flags missing Hebrew excerpts', () => {
  it('flags the original reported case (verse ref → quoted English follows)', () => {
    const text =
      "Tehillim 119:62 states, 'At midnight I will rise to give thanks to You,' describing David's practice";
  ...
  it('flags the second reported case (verse ref → English in parens)', () => {
    const text =
      "juxtaposes this pasuk with Tehillim 119:148 ('My eyes preceded the watches') to resolve";
    const issues = lintSynthesis(text);
    expect(issues.length).toBe(1);
...
describe('lintSynthesis — book-name disambiguation + position', () => {
  ...
  it('reports the verse=0 sentinel when only a chapter is cited', () => {
    const text =
      "the long acrostic of Tehillim 119 begins 'Happy are those whose way is blameless'";
    const issues = lintSynthesis(text);
    expect(issues.length).toBe(1);
  ...
  it('reports the byte offset of each match for highlighting', () => {
    ...
    const issues = lintSynthesis(text);
    expect(issues.length).toBe(1);
    expect(text.slice(issues[0].index, issues[0].index + issues[0].match.length)).toBe(
      'Tehillim 119:62',
    );
  });
  ...
  it('does not match a partial-word collision (Tehillim inside a longer string)', () => {
    const text =
      "the Tehillimkop scrollwork — 'a decorative motif' — appears in 4:5 of the manuscript";
    expect(lintSynthesis(text)).toEqual([]);
  });
});
...
describe('lintCalques — flags known calques', () => {
  it('flags the original Chulin 21a failure: "without most flesh"', () => {
    const text =
      "The Gemara resolves the objection that Eli's broken neck occurred without most flesh by distinguishing old age (זקנה שאני).";
  ...
  it('flags the padded variant "severing most of the flesh"', () => {
    const text =
      "Rashi explains that Eli's advanced age weakened his bones, so the neck snapped without the usual requirement of severing most of the flesh.";
    const issues = lintCalques(text);
    expect(issues.length).toBe(1);
  ...
  it('flags "majority of the flesh"', () => {
    const text =
      'The threshold requires breaking the spine plus the majority of the flesh torn with it.';
    const issues = lintCalques(text);
    expect(issues.length).toBe(1);
    expect(issues[0].hebrew).toBe('רוב בשר');
  });
  ...
  it('flags multiple distinct calques in one paragraph', () => {
    const text =
      'the house of justice ruled on whether a son of his year qualifies, citing the seven commandments of the sons of Noah';
    const issues = lintCalques(text);
