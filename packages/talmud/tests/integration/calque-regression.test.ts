...
interface RunOkResponse { status: 'ok'; result: { parsed: unknown } }
interface RunPendingResponse { status: 'pending'; runId: string }
interface RunErrorResponse { status: 'error'; error: string }
...
describe(`integration: argument-move.synthesis calque regression on Chulin 21a (against ${BASE_URL})`, () => {
  ...
  beforeAll(async () => {
    ...
    expect(out?.instances?.length ?? 0, 'expected argument-move instances on Chulin 21a').toBeGreaterThan(0);
    ...
    eliMoves = (out.instances ?? []).filter((m) => {
      ...
      return (
        /\beli\b|neck|spine|broken|flesh|elderly|aged/.test(en)
        || /מפרקת|רוב בשר|זקנה|עלי/.test(he)
      );
    });
  }, POLL_TIMEOUT_MS);
  ...
  it('argument-move.synthesis output for those moves contains no calque phrases', async () => {
    const failures: string[] = [];
    const sampled = eliMoves.slice(0, MAX_MOVES_TO_TEST);
    for (const move of sampled) {
      const out = (await runEnrichment('argument-move.synthesis', move, /* bypassCache */ true)) as { synthesis?: string } | null;
    ...
    expect(failures, `calques detected in fresh argument-move.synthesis output:\n${failures.join('\n\n')}`).toEqual([]);
  }, POLL_TIMEOUT_MS);
});
