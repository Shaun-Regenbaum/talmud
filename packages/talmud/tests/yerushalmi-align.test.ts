...
describe('yerushalmiRefToSefaria', () => {
  it('builds a same-tractate Sefaria ref from perek:halachah', () => {
    expect(
      yerushalmiRefToSefaria(
        { raw: 'Yerushalmi Perek 1 Halachah 1 Daf 1a', kind: 'yerushalmi', detail: '1:1' },
        'Berakhot',
      ),
  ...
  it('maps a cross-tractate ref to its Sefaria spelling', () => {
    expect(
      yerushalmiRefToSefaria(
        { raw: 'Yerushalmi Terumos ...', kind: 'yerushalmi', tractate: 'Terumos', detail: '4:1' },
        'Chullin',
      ),
  ...
  it('returns undefined without a perek:halachah', () => {
    expect(
      yerushalmiRefToSefaria({ raw: 'x', kind: 'yerushalmi', detail: 'Halachah 7' }, 'Bava Metzia'),
    ).toBeUndefined();
  });
});

describe('flatten + align Yerushalmi outline to Bavli segments', () => {
  const body = parseDafyomiContent('yerushalmi', fixture('berachos-yerushalmi-002.htm')).blocks[0]
    .body;
...
describe('yerushalmiFloorGroups — guaranteed anchors from strong verbatim runs', () => {
  const pt = (
    ...
    score: number,
    ref: string,
    extra: Partial<YerushalmiOutlinePoint> = {},
  ...
  it('merges adjacent same-ref strong points into one span, splits on a gap or a new ref', () => {
    ...
    expect(groups.map((g) => [g.startSegIdx, g.endSegIdx, g.yerushalmiRef])).toEqual([
      [0, 1, 'JT A 1:1'],
      [6, 6, 'JT A 1:1'],
      [7, 7, 'JT A 2:1'],
    ]);
