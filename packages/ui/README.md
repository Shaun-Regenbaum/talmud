...
- Nodes can carry `role`, `badge`, `badgeColor`, `color`, `selected`, and `dimmed`. Optional `description` and `annotation` reserve extra space for rulings and practice. The compact view shortens long labels and descriptions visually; `detail` keeps the full text available on selection in the modal.
